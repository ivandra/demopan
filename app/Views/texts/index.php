<h2>Texts: <?= htmlspecialchars($site['domain']) ?></h2>
<p style="font-size:13px;opacity:.85;">
    config.php генерируется в: <code><?= htmlspecialchars($configTargetPath) ?></code>
    | <a href="/sites/files/edit?id=<?= (int)$site['id'] ?>&file=config.php">открыть в Files</a>
</p>

<p>
    <a href="/">← назад</a>
</p>

<form method="post" action="/sites/texts/new?id=<?= (int)$site['id'] ?>">
    <label>Новый файл</label>
    <input name="new_file" placeholder="new.php">
    <button type="submit">Создать</button>
</form>

<hr>

<?php if (!$files): ?>
    <p>Файлов в texts пока нет.</p>
<?php else: ?>
    <table>
        <tr>
            <th>Файл</th>
            <th>Действия</th>
        </tr>
        <?php foreach ($files as $f): ?>
            <tr>
                <td><code><?= htmlspecialchars($f) ?></code></td>
                <td>
                    <a href="/sites/texts/edit?id=<?= (int)$site['id'] ?>&file=<?= rawurlencode($f) ?>">Открыть</a>
                    |
                    <form method="post"
                          action="/sites/texts/delete?id=<?= (int)$site['id'] ?>"
                          style="display:inline"
                          onsubmit="return confirm('Удалить файл <?= htmlspecialchars($f) ?>?');">
                        <input type="hidden" name="file" value="<?= htmlspecialchars($f) ?>">
                        <button type="submit">Удалить</button>
                    </form>
                </td>
            </tr>
        <?php endforeach; ?>
    </table>
<?php endif; ?>
