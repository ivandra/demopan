    ...
    public function createSite(): void
    {
        ...
        try {
            ...
            try {
				...
				// если certId уже известен — запишем, что сертификат есть, но не готов
				$this->updateSslState($siteId, isset($certId) && $certId > 0, isset($certId) ? (int)$certId : 0, false, $e->getMessage());
	...
	private function ftpCanWrite(string $host, int $port, string $user, string $pass): bool
{
    ...
    $tmpLocal = tempnam(sys_get_temp_dir(), 'ftpchk_');
    if ($tmpLocal === false) {
        @ftp_close($conn);
        return false;
    }

    @file_put_contents($tmpLocal, "ok\n");
...
public function updateFiles(): void
{
    ...
    try {
        ...
        if ($buildRel === '') throw new RuntimeException('build_path empty for this site');

        $buildAbs = __DIR__ . '/../../' . ltrim($buildRel, '/');
        if (!is_dir($buildAbs)) throw new RuntimeException('buildAbs not found: ' . $buildAbs);
        ...
        $ftpUser = (string)($site['fp_ftp_user'] ?? '');
        $ftpPass = '';
        ...
        $tmpUnpacker = sys_get_temp_dir() . '/hub_unpack_' . $siteId . '_' . $deployId . '.php';
    ...
    public function issueSslSelfSigned(): void
{
    ...
    try {
        ...
        if ($currentCertId > 0 && $currentEnabled === false) {
            ...
            $job = $client->waitQueueSslCertificateJob($fpSiteId, $certId, 120, 2);
        ...
        // Если certId уже успели получить — хотя бы отметим ошибку в ssl_error
        try {
    ...
    for ($i = 1; $i <= $maxAttempts; $i++) {
        ...
        try {
            ...
            // IMPORTANT: таймауты на data-канал
            ...
            // IMPORTANT: PASV включаем ПОСЛЕ логина
            @ftp_pasv($conn, true);

            // Чуть стабилизирует некоторые FTP
            @ftp_chdir($conn, '.');
...
private function waitSslReadyByProbe(string $domain, int $timeoutSec = 120, int $pollSec = 3): array
{
    ...
    while (true) {
        ...
        if (($last['ok'] ?? false) === true) {
            // можно дополнительно проверить valid_to > now-...
            return $last;
        }
...
private function isResolvable(string $domain): bool
{
    ...
    // если это уже IP — "резолвится"
    if (preg_match('~^(?:\d{1,3}\.){3}\d{1,3}$~', $domain)) return true;

    // DNS check
    $ip = @gethostbyname($domain);

    // gethostbyname возвращает сам домен, если не смог резолвить
